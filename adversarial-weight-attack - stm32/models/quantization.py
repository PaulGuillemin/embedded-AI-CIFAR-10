...
class quan_Conv1d(nn.Conv1d):
    ...
    def forward(self, input):
        if self.inf_with_weight:
            return F.conv1d(input, self.weight * self.step_size, self.bias,
            ...
            return F.conv1d(input, weight_quan, self.bias, self.stride,
    ...
    def __reset_weight__(self):
        ...
        with torch.no_grad():
            self.weight.data = quantize(self.weight, self.step_size,
                                        self.half_lvls)
        # enable the flag, thus now computation does not involve weight quantization
