...
def evaluate_model_on_STM32(debut,iterations, serial_port):
    ...
    for i in range(debut,iterations):
        ...
        print(f"   Expected output: {Y_test[i]} -- {np.argmax(Y_test[i])}")
        print(f"   Received output: {output} -- {np.argmax(output)}")
        print(f"----------------------- Accuracy: {accuracy:.2f}\n")
    return accuracy


if __name__ == '__main__':
    X_test = np.load("C:\\Users\\marcs\\OneDrive\\Documents\\Cours\\Semestre 9\\EMSE - Cours\\IA_Embarquee\\Initial_model_dataset\\Model_initial\\CIFAR10_xtest.npy")
    Y_test = np.load("C:\\Users\\marcs\\OneDrive\\Documents\\Cours\\Semestre 9\EMSE - Cours\\IA_Embarquee\\Initial_model_dataset\\Model_initial\\CIFAR10_ytest.npy")
